...
models_path = os.path.join(base_path, "assets")
saved_models_path = os.path.join(models_path, "pose_model")
...
class FaceDetector:
    ...
    def __init__(
        self,
        dnn_proto_text=os.path.join(models_path, "deploy.prototxt"),
        dnn_model=os.path.join(models_path, "res10_300x300_ssd_iter_140000.caffemodel"),
    ):
        """Initialization"""
    ...
    def draw_all_result(self, image):
        ...
        for facebox, conf in self.detection_result:
            cv2.rectangle(
                image, (facebox[0], facebox[1]), (facebox[2], facebox[3]), (0, 255, 0)
            )
            label = "face: %.4f" % conf
            ...
            cv2.rectangle(
                image,
                (facebox[0], facebox[1] - label_size[1]),
                (facebox[0] + label_size[0], facebox[1] + base_line),
                (0, 255, 0),
                cv2.FILLED,
            )
            cv2.putText(
...
class MarkDetector:
    ...
    def __init__(self, saved_model=saved_models_path):
        """Initialization"""
